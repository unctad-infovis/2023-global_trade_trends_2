...
function Figure1() {
  ...
  useEffect(() => {
    const data_file = `${(window.location.href.includes('unctad.org')) ? 'https://storage.unctad.org/2023-global_trade_trends_2/' : './'}assets/data/2023-global_trade_trends_2_figure1.csv`;
    try {
      fetch(data_file)
        .then((response) => {
          if (!response.ok) {
  ...
  return (
    <div className="app">
      ...
      <ChartLine
        ...
        subtitle="Trends for trade in goods and services, quarterly growth, 2019–2023 Q2"
        suffix="%"
